export default class Tabby {
  constructor(tabby) {
    ...
    this.contentsWrapper = tabby.querySelector('.tabs-contents');
    ...
    this.contentsWrapper.addEventListener('click', this.handleBackToTabs)
  }
   ...
   */
  getTabpanel(id) {
    return this.tabby.querySelector(`[aria-labelledby="${id}"]`);
  }
  ...
  //Shifts focus from Back button to Tab
  handleBackToTabs = (event) => {
    const btn = event.target.closest('.back-to-tabs');
    if (!btn) return;

    const tabId = btn.dataset.tabId;
    const tab = this.tabby.querySelector(`#${tabId}`);
    if (tab) tab.focus();
  };
